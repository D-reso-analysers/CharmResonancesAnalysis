...
def mass_selections(df, mass_min0, mass_max0, mass_min1, mass_max1, cpa_min1, radius_min1):
    dfOut = pd.DataFrame()
    dfOut = df[(df['fMBach0'] >= mass_min0) &
               (df['fMBach0'] <= mass_max0) &
               (df['fMBach1'] >= mass_min1) &
               (df['fMBach1'] <= mass_max1) &
               (df['fCpaBach1'] >= cpa_min1) &
               (df['fRadiusBach1'] >= radius_min1)]
    return dfOut
...
if __name__ == "__main__":
    ...
    if cfg['other_cuts']['optionalRawYieldOutput']:
        rawYieldOutputFile = ROOT.TFile(cfg['other_cuts']['optionalRawYieldOutput'], "RECREATE")
    else:
        rawYieldOutputFile = None
    outfile = ROOT.TFile(cfg['other_cuts']['outputFile'], "RECREATE")
    ...
    dfDataCentralCut = applySelections2(dfDataFiltered, centralCuts, varsToSkip = ['invMassProng0', 'invMassProng1', 'cpaBach1', 'radiusBach1'])
    dfMcRecCentralCut = [applySelections2(dfRec, centralCuts, varsToSkip = ['invMassProng0', 'invMassProng1', 'cpaBach1', 'radiusBach1'], isMC=True, selectedFlags=flagsToKeep) for irec, dfRec in enumerate(dfRecList)]
    ...
    variations = cfg['other_cuts']['variations']
    mass_min0s = variations['mass_min0']
    mass_min1s = variations['mass_min1']
    mass_max0s = variations['mass_max0']
    mass_max1s = variations['mass_max1']
    cpa_min1s = variations['cpa_min1']
    radius_min1s = variations['radius_min1']
    

    nvars = len(mass_min0s)*len(mass_max0s)*len(mass_min1s)*len(mass_max1s)
    ...
    for ipt, (pt_min, pt_max) in enumerate(zip(pt_mins, pt_maxs)):
        ...
        for mass_min0 in mass_min0s:
            for mass_max0 in mass_max0s:
                for (mass_min1, mass_max1) in zip(mass_min1s, mass_max1s):
                    for (cpa_min1, radius_min1) in zip (cpa_min1s, radius_min1s):
                        ...
                        dfDataSelPt = mass_selections(dfDataPt, mass_min0, mass_max0, mass_min1, mass_max1, cpa_min1, radius_min1)
                        mcRecSelPtList = [mass_selections(dfRec, mass_min0, mass_max0, mass_min1, mass_max1, cpa_min1, radius_min1) for dfRec in mcRecPtList]
                        # 2) efficiency
